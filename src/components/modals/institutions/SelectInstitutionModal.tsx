...
const SelectInstitutionModal = (props: any) => {
    ...
    const renderInstitution = (data: any) => {
        return (
            <TouchableOpacity
                ...
                style={selectedList?.map((v: any) => v?._id)?.includes(data?.institute?._id) ? styles.selectedInstContainerStyle : styles.instContainerStyle}
                onPress={() => {
            ...
            >
                <AvatarCmp
                    ...
                    size={30}
                />
                <Text style={[styles.instTextStyle, { color: selectedList?.map((v: any) => v?._id)?.includes(data?.institute?._id) ? colors.white : colors.primary }]}>{data?.institute?.name}</Text>
            </TouchableOpacity>
        )
    }
    const renderPartner = (data: any) => {
        return (
            <TouchableOpacity
                key={`institution_${data?._id}`}
                style={selectedList?.map((v: any) => v?._id)?.includes(data?._id) ? styles.selectedInstContainerStyle : styles.instContainerStyle}
                onPress={() => {
            ...
            >
                <AvatarCmp
                    ...
                    size={30}
                />
                <Text style={[styles.instTextStyle, { color: selectedList?.map((v: any) => v?._id)?.includes(data?._id) ? colors.white : colors.primary }]}>
