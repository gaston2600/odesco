...
const MembersListModal = (props: any) => {
    ...
    const { members, pendings, requests } = useSelector((state: any) => state?.Network)
    ...
    function memberType(params: any) {
        if (members?.map((v: any) => v?.user?._id)?.includes(params)) return "member"
        if (pendings?.map((v: any) => v?.user?._id)?.includes(params)) return "pending"
        if (requests?.map((v: any) => v?.user?._id)?.includes(params)) return "invitation"
        return "add"
    }
    ...
    function getPage() {
        ...
        dispatch(getPartnersList(
            ...
            (res: any) => {
                setLoading(false)
                setPartners(res?.partners?.filter((v: any) => v?._id !== defaultPartner))
            },
            (err: any) => {
                setLoading(false)
                console.log({ err });
        ...
        >
            <View style={styles.containerStyle}>
                ...
                <View style={styles.searchContainerStyle}>
                    ...
                    >
                        <Icons.AntDesign name={!!searchInput ? "close" : "search1"} size={20} color={!!searchInput ? colors.black : colors.grey} />
                    </Pressable>
