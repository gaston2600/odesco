...
const LikePostCmp = (props: any) => {
  ...
  function hasLike() {
    return data?.likes?.some(
      (v: any) =>
        v?.partner?._id === selectedSpace?._id ||
        v?.institution?._id === selectedSpace?._id,
    );
  }
  hasLike();
  return (
    <TouchableOpacity
      ...
      <SelectInstitutionModal
        ...
        selectedList={data?.likes?.map((v: any) =>
          v?.institution?._id
            ? {_id: v?.institution?._id, type: 'Institution'}
            : {_id: v?.partner?._id, type: 'Partner'},
        )}
      />
    </TouchableOpacity>
  );
